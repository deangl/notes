...
* 对于群$G$的子群$H$，$N_G(H) := \{a\in G \vert aHa^{-1} = H\}$，称为**正规化子群**，有$N_G(H) < G, H \triangleleft N_G(H)$。
...
* 如果$N \triangleleft G$，且$N, G/N$ 都可解，那么$G$ 可解(显然可以先解到$N$，再解到$\{e\}$)。
...
* **$\mathrm{Jordan-H\ddot{o}lder}$ 定理：** 有限群$G$的任意两个无重复项的合成群列有同样的长度，且其因子群能用某种方法配对，使得对应的因子群是同构的。
  证明用到归纳法，主要思路看看长度$2$的情况：如果$G \triangleright G_1 \triangleright \{e\}; G \triangleright H_1 \triangleright \cdots$。那么如果$G_1 \ne H_1$有：
  * $G_1H_1 = G$，因为这是$G$的一个大于$G_1$的正规子群，而$G/G_1$是单群，那么它只能是$G$。
  * $G_1 \cap H_1 = \{e\}$，这是因为 $G_1 \cap H_1 \triangleleft G_1$，而$G_1$是单群，它只能是$\{e\}$
...
* **轨道：** 任取$x \in \Omega$，定义$G(x) := \{a\circ x \vert a\in G\}$，称$x$的$G-$轨道。这对$\Omega$形成一个划分。
* **稳定子群：** $G_x := \{g\in G \vert g\circ x = x\}$，$G_x < G$。而且有$aG_x = bG_x \Leftrightarrow a\circ x = b\circ x$。
* **轨道-稳定子定理：** $\vert G(x) \vert = [G:G_x]$。
![](./img/1721624700.png)
* 群$G$在$\Omega := \{H \vert H < G \}$ 上的作用$aH := aHa^{-1}, a\in G$称为共轭作用。这个作用下$H$的$G-$轨道是$H$的所有共轭子群的集合，对应的**稳定子群**是**正规化子群**$N_G(H)$。有共轭子群的个数为$[G: N_G(H)]$。
* 设群$G$在$\Omega$上有一个作用，则同一$G-$轨道上的点的稳定子群彼此共轭，从而这些稳定子群的基类相同。$y=a\circ x \Leftrightarrow G_y = aG_xa^{-1}$。这有一个有意思的推论：一条$G-$轨道里所有元素的稳定子群的阶之和就是$\vert G \vert$。
![](./img/1722300461.png)
* **群元素的不动点集：** $F(g) := \{ x\in \Omega \vert g\circ x = x \}$；**群的不动点集：** $\Omega_0 :=\{x \in \Omega \vert G(x) = \{x\}\}$。
