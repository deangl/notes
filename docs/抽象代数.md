...
* **一个高斯整环$R$和他的分式域$F$，有$R[x]$ 中有次数大于$0$的本源多项式$g(x)$ 在$F(x)$中不可约 $\Leftrightarrow g(x)$ 在$R[x]$中不可约**。证明过程有好几个中间结论：
  * $f(x) = \frac{d}{c} g(x), d,c \in R$，其中$g(x)$是$R[x]$中的本源多项式(可以把不属于$R$的系数 **提出来**)，且在$R[x]$中相伴的意义下$g(x)$是$f(x)$唯一决定的。证明可以令$d$为$f(x)$的系数的分母的一个最小公倍数(通分)。然后证在相伴下唯一，这又要用到：
