...
在衍生品定价中，$\sigma$是一个很特殊的参数：漂移项可以在测度变化中改变，但$\sigma$是保持不变的。这个不变性是一个很重要的点，使得我们不能随意选择$\sigma$，从而估值是可靠的。
...
$$
\begin{eqnarray*}
V_t +rS_tV_x+\frac{1}{2}\sigma^2 S_t^2 V_{xx}-rV_t =0
\end{eqnarray*}
$$
...
$$
有：
若设函数 $f(t,x)$ 的偏导数 $f_t(t,x),f_x(t,x),f_{xx}(t,x)$ 都有定义并且连续，则：
$$
\begin{eqnarray*}
\mathrm{d} f(t,X_t) = f_t(t,X_t)\mathrm{d}t + f_x(t,X_t)\mathrm{d}X_t + \frac{1}{2} f_{xx}(t,X_t) \mathrm{d}[X_t,X_t]
\end{eqnarray*}
$$
其中 $\mathrm{d}[X_t,X_t] = \Delta(t)^2 \mathrm{d}t$，结果是明确的，且与测度无关。
...
**ちょっと待ってください**，这样就行了吗？需要注意，这里我们只拿到**一个**变换，有没有其它“放缩”过的测度变换，也能使$(V_t/B_t)$可是鞅呢？即一个改变布朗运动前的系数$\sigma$的。很显然，根据前一节PDE解释下的结论，应该是不行的。但网上没找到对此的表述，我们自己来证一下。
...
对于这个问题的直观理解是，在一个标准布朗运动中，$T$时点运动到任一位置都是可以接受的，但是在很多个小时段$\Delta t_i$内，变动平方不是$\Delta t_i$这种情况是不可接受的，改测度也不行。
...
$\frac{Q(X_T<1)}{P(X_T<1)} = \frac{\Phi(1/2)}{\Phi(1)} \approx 0.82$

$\frac{Q(X_{T/2}<1/\sqrt{2},X_{T}-X_{T/2}<1/\sqrt{2})}{P(X_{T/2}<1/\sqrt{2},X_{T}-X_{T/2}<1/\sqrt{2})} = 0.82^2$

……

$\frac{Q(X_{T/n}<1/\sqrt{n},...,X_{T}-X_{(n-1)T/n}<1/\sqrt{n})}{P(X_{T/n}<1/\sqrt{n},...,X_{T}-X_{(n-1)T/n}<1/\sqrt{n})} = 0.82^n\rightarrow 0$。
...
* 在前式中，注意 $\theta_t \Delta t +\frac{1}{2}\Gamma_{t+\Delta t} (S_{t+\Delta t}-S_{t})^2$这一项，对比$\theta_t \Delta t +\frac{1}{2}\Gamma_{t+\Delta t} S_t^2\sigma^2\Delta t = 0$的$Parity$，如果$|S_{t+\Delta t}-S_{t}| > \sigma S_t\sqrt{\Delta t}$的话，组合在这一段时点是会赚钱的，反之亦然。如果我们对于$\sigma$的估计和实现的结果比较“一致”，这里“一致”指的是，在一条路径上：$\sum ((S_{t+\Delta t}-S_{t})^2 -\sigma^2 S_t^2 \Delta t) \sim 0$，那么，整体的盈亏会接近$0$。所以如果$\sigma$的估计比实现要大，那么整体会出现盈利，反之亦然。
...
对于问题2)，可以看看下面的两张图，图中蓝线代表了买入Call在对冲时参数和实现波动率一致时，如何在对冲端赚到原始期权的价值的。

![](./img/1658209218.png)

形象地说，可以解释为这种情况注定了“反复横跳”(ATM说明要跳回来的)。而“反复”的过程中，存在对冲头寸大小和“下一步”涨跌的正相关，在高仓位的时候多赚了，低仓位的时候少赔些。我们需要“骑乘”$\Gamma$来赚钱。
...
举个例子吧：这个结论下，带均值回复属性的过程[Ornstein–Uhlenbeck 过程](https://en.wikipedia.org/wiki/Ornstein%E2%80%93Uhlenbeck_process)，$dx_t = \theta(\mu - x_t)dt + \sigma d W_t$。这个过程在长时间后的分布会收敛为以$\mu$为中心的高斯分布 $\frac{\sigma}{\sqrt{2\theta}} W_1$，即 $N(\mu,\frac{\sigma^2}{2\theta})$。很明显，这和不收敛的简单布朗运动是不一样的，显然，对于一个pay off在这两个分布下求的期望是不一样的，需要注意，不能按这个过程的分布来直接求期望估值。因为它不是鞅。

Q：那我们记得有的公式中是有“漂移”项的，是什么呢？又是怎么回事呢？
A：注意，那些项是些 **repo** 或 **dividend** 这样的，这些东西对应的不是“漂移”，而是在标的价格之外的 **现金流**。
