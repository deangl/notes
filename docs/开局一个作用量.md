...
* 作用量在真实运动下取极小值即为运动方程，那么$L(q,v)$可以被确定到一个什么程度呢？可以看出，作用量如果加上一个$q,t$的函数$g(q,t)$，那么会由于运动的端点不变，这一项对运动方程没有贡献。这对应$L_2(q,v) = L(q,v) + \frac{\mathrm{d}g}{\mathrm{d}t}$。
* 各向同性要求$L$是速度大小的函数，所以是$v^2$的函数，会有问题为什么不是**绝对值或4次方** 。然而事实上，不论是**绝对值**还是**4次方**都还是$v^2$的函数。这里的平方项不是“随便”选的，而是来自于平直空间的自然**距离**中来的 ———— 因为平直空间的距离是2-范数。
* 令$v^2=k$，在$v^{\prime} = v+\epsilon$时，有$k^{\prime} = k+2\epsilon \sqrt{k}$，有$L^{\prime}= L(k^{\prime}) = L(k) + 2 \frac{\partial L}{\partial k}\epsilon \sqrt{k} = L(k) + 2 \frac{\partial L}{\partial v^2}\epsilon v$。根据前面的关于$L$的确定程度的要求，这里的第二项应可以表示为 $ \frac{\mathrm{d} f(q,t)}{\mathrm{d} t} $的形式：$2\frac{\partial L}{\partial v^2}  \epsilon \frac{\mathrm{d} q}{\mathrm{d} t} = \frac{\mathrm{d} f(q,t)}{\mathrm{d} t}$。这里$2\frac{\partial L}{\partial v^2}$如果不是常数，只能是$v$的函数，$g(v) \frac{\mathrm{d} q}{\mathrm{d} t}$不会是全微分(它对$t$积分的结果和路径有关)。于是$2\frac{\partial L}{\partial v^2}$只能是常数。
...
* 书上有一个为什么作用量$S$是不变量的论证，其中有两点：1)正的间隔在另一个系下还是正的；2)$\delta S$只和系的相对运动的大小有关，而和方向无关。没有展开说明，这里要补一下：
  * 正的间隔是代表可以有因果联系的，这和参考系无关，所以正的间隔还是正的。
  * 第二点的证明比较复杂：
	1. 换一个参考系应该对应一个基的线性变换。计$x,y$在一个系下的标量积为$g(x,y), g(x):=g(x,x)$，变换后新系下的标量积$G(x,y)$。需要证明的是，如果有$\forall x, g(x)=0 \Rightarrow G(x)$，那么对于给定的变换，有一个固定的$k \in R$使得 $G(x) = kg(x)$。
	2. 只需证明对于 $\forall x _1 , x _2, g(x _1) = g(x _2) = 1$ 有 $G(x _1) = G(x _2)$。
	3. 容易看出平面 $x _1,x _2$ 和光锥有两条交线，也就是 $g(x _1 + a x _2) = 0, g(x _1 + b x _2) = 0, a\ne b$。那么：
$$
\begin{eqnarray*}
 &g(x _1) + a^2 g(x _2) + 2a g(x _1, x _2) = 0 \\
 &g(x _1) + b^2 g(x _2) + 2b g(x _1, x _2) = 0 \\
 &1 + a^2 + 2a g(x _1, x _2 ) = 0 \\
 &1 + b^2 + 2b g(x _1, x _2 ) = 0 \\ 
 \Rightarrow & \frac{1+a^2}{1+b^2} = \frac{a}{b}
\end{eqnarray*}
$$	
	4. 同时有$G(x _1 + a x _2) = 0, G(x _1 + b x _2) = 0$，从而：
$$
\begin{eqnarray*}
 &G(x _1) + a^2 G(x _2) + 2a G(x _1, x _2) = 0 \\
 &G(x _1) + b^2 G(x _2) + 2b G(x _1, x _2) = 0 \\
 \Rightarrow & \frac{G(x _1) + a^2 G(x _2)}{G(x _1) + b^2 G(x _2)} = \frac{a}{b} = \frac{1+a^2}{1+b^2} \\
 \Rightarrow & a^2 G(x_1) + b^2 G(x_2) =  b^2 G(x_1) + a^2 G(x_2) \\
