...
先简单的看一个两步两状态过程。假设有现金流 $1$ 分别发生在时间 $t_1$ 和时间 $t_2, 0<t_1<t_2$，这两笔现金流在 $0$ 时刻有市场价格 $p(0,t_1),p(0,t_2)$。计$\mathscr{r}(d,t)=\frac{-\mathrm{ln}(d)}{t}$($d$ for discount)。我们会分别称$\mathscr{r}(p(0,t_1),t_1),\mathscr{r}(p(0,t_2),t_2)$为即期利率。
...
结合前面提到的$p(0,t_n)$的式子，可以取$r_s(t) := \lim \limits_{\delta t \to 0} \frac{1 - p(t, t+\delta t)}{\delta t}$
...
\end{eqnarray*}
$$
这样：$p(t+\mathrm{d}t,T)(1-r_s(t)\mathrm{d}t)$的过程是**无漂移的**，应该有：
$$
\begin{eqnarray*}
...
\end{eqnarray*}
$$
这个是显然的哈，看着很熟悉，同股票的形式是一样的。事实上，如果面对的是一个比较长期限的债券的期权，这基本上就够了(解决了风险中性的问题)。然而，这里存在下面两个主要问题：
...
\end{eqnarray*}
$$
到这一步，得到了建模量$f$和观测量$p$的关系。而另一方面，注意前式到风险中性的要求：
$$
\begin{eqnarray*}
\mathrm{d} \ln(p(t,T)) &=& (r_s(t)-\frac{1}{2}\sigma_p^2(t,T))\mathrm{d}t + \sigma_p(t,T) \mathrm{d}W
\end{eqnarray*}
$$
对比得(注意到前面的$\mathrm{d}W$是属于$p$的，且是对不同的$T$不一样的，下面记为$\mathrm{d} W_{pT}$，以明确)：
$$
\begin{eqnarray*}
-\frac{1}{2}\sigma_p^2(t,T)\mathrm{d}t + \sigma_p(t,T) \mathrm{d}W_{pT} &=& -\int \limits_{t}^{T} \mathrm{d}f(t,u) \mathrm{d}u
\end{eqnarray*}
$$

注意，理论到这个时侯其实已经可以完结了，我们已经可以在风险中性测度下拟合任意的债券价格波动。不过，可以进一步分析一下$\mathrm{d}W_{pT}$。
1. 我们并不希望对所有的$T$，$W_{pT}$是那么无关联的，比如，靠近的$T$，$W_{pT}$也应该靠近。
2. 我们的观测量是有限的，也不需要那么任意的$W_{pT}$。

可以对于$f(t,u)$的行为再做一点假设了，比如认为对于利率的影响，只有$n$个独立的因素(此处逻辑冠冕堂皇，实际上，是为了拟合的时候有足够的自由度)，进一步这$n$个因素分别是布朗运动，设：$\mathrm{d}f(t,u) = \mathbf{\sigma}(t,u) \cdot \mathbf{\mathrm{d} W_{ft}} + \mu(t,u) \mathrm{d}t$。

先来感受一下这个式子的意义：$0$时点的时候，$f(t,u)$是确定的，之后，随机增量只是对于参数$t$的。具体增量多少是由$n$个布朗运动的实现$\mathbf{\mathrm{d} W_{ft}}$，通过$\mathbf{\sigma}(t,u)$加权得到的。
...
$$
\begin{eqnarray*}
\sigma_p(t,T) &=& \vert \int \limits_{t}^{T} \sigma(t,u) \mathrm{d}u \vert \\
...
\end{eqnarray*}\tag{p-f relation}\label{p-f}
$$

其中$\vert \int \limits_{t}^{T} \sigma(t,u) \mathrm{d}u \vert$是一个积分的向量的模。对$T$求导：
$$
\begin{eqnarray*}
...
* 置$\sigma = 0$，有$f(t,T)$不随$t$变化的，注意这和“固定tenor上的点保持不变”、亦即“ride the curve”策略基准的形式是不一样的($T$是固定的时点，不是tenor)，即不允许有“确定的，且时间平移不变的曲线”，这是合理的，因为本质上“ride the curve”是有套利的，是不应该发生的。
* 取$n=1$，$\sigma(t,T) = \sigma$为常函数，有$\mathrm{d}f(t,T) = \sigma^2 (T-t) \mathrm{d}t + \sigma \mathrm{d}W$：是一个有正向漂移，且越远端漂移越快的过程。当然，我们也可以调整$\sigma(t,T)$的型式，如$\sigma(t,T) =  (T-t)^{-\gamma}\sigma$，会使得远端的漂移稍慢，但整体会有“近端波动大，漂移小；远端波动小、漂移大”的情况，往往使得曲线越变越陡。(不要害怕，这只是风险中性测度下的东西，不代表真实世界的漂移)。

## 利率作为期权标的的合法性

先要明确，什么叫 **合法** 。它指的是：若市场的行为符合模型的预期，那么可以通过对于基础资产的交易，完全复制衍生品的payoff。或者说，对于期初净值为$0$的投资者，可以通过融资买入一个衍生品，并融资(投资)于零息票，最后实现$0$的损益(这个定义更干净)。而衍生品的payoff，应该是指的一个函数$E \to \mathrm{R}$，其中$E$ 是**利率**的有限个观察值的集合。所以问题就简化成了，任意一个**利率**，是不是都可以用$p(t,u)$的组合来表达，而往往这是可以满足的。
