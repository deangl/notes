...
&\mathcal{L}:=\frac{1}{r(x)}(\frac{{\rm d}}{{\rm d}x} (p(x)\frac{{\rm d}}{{\rm d}x})+q(x))
\end{eqnarray*}
$$
其中$\mathcal{L}$很奇怪，而它对应的其实就是工$D_{(2)}:=a\frac{{\rm d^2}}{{\rm d}x^2}+b\frac{{\rm d}}{{\rm d}x}+c$。写成这个样子是方便一路积分上来。
$$
\begin{eqnarray*}
r(x)&=&\frac{1}{a(x)}e^{\int_{x_0}^x\frac{b(s)}{a(s)}{\rm d}s}\\
p(x)&=&e^{\int_{x_0}^x\frac{b(s)}{a(s)}{\rm d}s}\\
q(x)&=&c(x)r(x)
\end{eqnarray*}
$$
