...
2. 可积的**连续**函数$f$可以被三角多项式一致逼近。即$\forall \epsilon > 0$，存在三角多项式$P$，使得$\vert f(x)-P(x)\vert<\epsilon$。
   * 证明要用到另一个核，**费叶核：** $F_N(x)=\frac{D_0(x)+···+D_{N-1}(x)}{N}$，它是傅利叶级数的**切萨罗和**。它的性质比狄利克雷核好：它是非负的，而且存在$M$ 对任意$n$有$\int \limits_{-\pi}^{\pi}\vert K_n(x)\vert\mathrm dx\leq M$。
...
$$
\begin{eqnarray*}
p(x)(y_m(x)\frac{\mathrm{d} }{\mathrm{d} x}y_n(x)y_n(x) - y_n(x)\frac{\mathrm{d} }{\mathrm{d} x}y_m(x)) \vert_a^b=0
\end{eqnarray*}
$$
其中$m,n$分别是不同$\lambda$下的解(本征函数)。那么，这些本征函数在$C^2$的函数空间中完备。


来点例子看看这个边界条件到底包含点什么：
* 齐次：在两个边界$i = 0,1$上都有 $\alpha_i y(x) + \beta_i y^{\prime}(x) = 0,~ \alpha_i^2+\beta_i^2 \ne 0$，特别的有
  * Dirichlet: $y_i = 0$
  * Neumann: $y_i^{\prime}$
