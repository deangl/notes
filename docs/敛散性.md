...
上面这两个用到了Abel变换(分部求和法)：
$$
\begin{eqnarray*}
\sum \limits_{i=m}^{n} f_i (g_{i+1} - g_{i}) = [f_{n+1} g_(n+1) - f_m g_m] - \sum \limits_{i=m}^{n} g_{i+1} (f_{i+1} - f{i})
\end{eqnarray*}
$$
...
* **[黎曼-勒贝格定理](https://zh.wikipedia.org/wiki/%E9%BB%8E%E6%9B%BC-%E5%8B%92%E8%B4%9D%E6%A0%BC%E5%AE%9A%E7%90%86)：** 如果$f$在区间$[a,b]$上可积，$g$为可积周期函数，且在一个周期上积分为零，则有$\lim \limits_{\lambda \to \infty} \int \limits_a^b f(x)g(\lambda x) \mathrm{d}x = 0$。
...
并且可以通过切萨罗和(这样会得到一个费耶核)的方法，得到一致收敛。下图对比了一下费耶核与狄利克雷核的区别。
...
2. 可积的**连续**函数$f$可以被三角多项式一致逼近。即$\forall \epsilon > 0$，存在三角多项式$P$，使得$\vert f(x)-P(x)\vert<\epsilon$。那么根据前一条，同阶的傅利叶部分和会收敛得更好。
