...
* **外微分:** 类似[微分](./切空间.md#微分)，它是一个映射$\mathrm{d} : \bigwedge^n (T_p^{\ast}M) \rightarrow \bigwedge^{n+1} (T_p^{\ast}M)$。使得：
 ...
 \rightarrow \mathrm{d}\varphi&=&\sum_{I}\sum_{i}\frac{\partial f_I}{\partial x^ni}\mathrm{d}x^i\wedge \mathrm{d}x^I
 \end{eqnarray*}
 $$
...
\end{eqnarray*}
$$
  考虑$\partial_j \partial_i = \partial_i \partial_i,  \mathrm{d} x^j \wedge \mathrm{d} x^i = -\mathrm{d} x^i \wedge \mathrm{d} x^j$，得证。
