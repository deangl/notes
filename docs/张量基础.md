...
据此，我们定义：
> 一个指标对象$T_{i_1...i_s}^{j_1...j_r}$称为一个$(r,s)$形张量，如果在基向量变换$e_i = S_i^j e^{\prime}_j$下，有 $ T_{i_1...i_s}^{\prime j_1...j_r} = S_{k1}^{j_1}...S_{k_r}^{j_r}(S^{-1})_{i_1}^{l_1}...(S^{-1})_{i_s}^{l_s}T_{l_1...l_s}^{k_1...k_r}$。
