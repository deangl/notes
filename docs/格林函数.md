...
$$
\begin{eqnarray*}
u(x) = \int f(x) G(x,s) \mathrm{d}s
\end{eqnarray*}
$$

这个太宽了，一般用的时候，常用在Strum-Liuville算子：$L = \frac{\mathrm{d}}{\mathrm{d}x} [p(x) \frac{\mathrm{d}}{\mathrm{d}x}] + q(x)$ 上。而给定边界算子：
$$
\begin{equation}
Du = \left\{
\begin{aligned}
...
$$
\begin{eqnarray*}
\int \limits_V (\phi \nabla^2 \psi - \psi \nabla^2 \phi) \mathrm{d}V = \int \limits_S (\phi \nabla \psi - \psi \nabla \phi) \cdot \mathrm{d}\hat{\sigma}
\end{eqnarray*}
$$
...
\begin{eqnarray*}
\phi(x) =
\int \limits_V G(x,x^{\prime}) \rho^{\prime} (x^{\prime}) \mathrm{d}^3 x^{\prime} +
\int \limits_S \phi(x^{\prime})\nabla^{\prime}G(x,x^{\prime})- G(x,x^{\prime})\nabla^{\prime}\phi(x^{\prime})\cdot \mathrm{d} \hat{\sigma}^{\prime}
\end{eqnarray*}
$$
...
$$
\begin{eqnarray*}
g(x,x^{\prime\prime}) &=& \int \limits_V G(x,x^{\prime}) \delta (x^{\prime},x^{\prime\prime}) \mathrm{d}^3 x^{\prime} +
\int \limits_S \phi(x^{\prime})\nabla^{\prime}G(x,x^{\prime})- G(x,x^{\prime})\nabla^{\prime}\phi(x^{\prime})\cdot \mathrm{d} \hat{\sigma}^{\prime} \\
