...
use_math: true
categories: [物理]
tags: [数学,物理,math,力学,哈密顿,哈密顿-雅可比方程,运动积分]
---

有几个东西，也不知道应该合到哪个专题，但是当年又没有理解好，包括有：

* 为什么搞出个哈密顿-雅可比方程（哈雅方程）
* 为什么要叫“运动积分”
...
$$
\begin{eqnarray*}
\delta \int \limits_{t_1}^{t_2} [p\dot{q} - H(q,p,t)] \mathrm{d}t = 0 \\
\delta \int \limits_{t_1}^{t_2} [P\dot{Q} - K(Q,P,t)] \mathrm{d}t = 0 \\
\end{eqnarray*}
$$
从而有：
...
首先要注意到一个问题，运动方程(拉格朗日方程、牛顿方程)中只出现到最高$q$对时间的二次导数，形式上是 $g_n(q_i^{\prime\prime};q_i^{\prime};q_i;t)=0$ 的形式。空间 $(q_i^{\prime\prime};q_i^{\prime};q_i;t)$ 是 $3s+1$ 维的，而轨迹是 $1$ 维的。$s$ 个方程把解约束在 $2s+1$ 维的曲面 $g_n=0$ 上，其上的一条 $1$ 维曲线还需要 $2s$ 个参数才能确定，这 $2s$ 个常数就对应 $2s$ 个运动积分。

再看具体是怎么“积分”的（注意：以下几步是示意性的计数论证，右端仍依赖未知函数，不能当作可直接积分的函数来严格积分；严格的依据是 ODE 理论——二阶方程组由 $2s$ 个初值决定解，对应 $2s$ 个一阶积分）：
...
* 应该有$q_n^{\prime\prime} = F_n(q_i^{\prime};q_i)$，有一个小技巧，$q_a^{\prime\prime} \mathrm{d}q_b = q_b^{\prime} \mathrm{d}q_a^{\prime}$。 两边都乘 $ \mathrm{d} q_1$ 得到$ \mathrm{d}q_n^{\prime} = \frac{F_n(q_i^{\prime};q_i)}{q_1^{\prime}}\mathrm{d}q_1$。
...
\end{eqnarray*}
$$
其中 $[f,H] := \frac{\partial f}{\partial q}\frac{\partial H}{\partial p} - \frac{\partial f}{\partial p}\frac{\partial H}{\partial q}$ 称为 $f$ 与 $H$ 的泊松括号。

而为什么要弄出这么个东西来？一层层说：

1. 先和上一节连起来：$[f,H] = 0$ 时 $\frac{\mathrm{d} f}{\mathrm{d} t} = \frac{\partial f}{\partial t}$，若 $f$ 再不显含 $t$，就有 $f$ 是守恒量。所以**泊松括号给出了找运动积分的判据**——找与 $H$ 对易的量就行。这比一节里靠"数维数"的办法实用多了。
2. 再想**正则变换**：正则变换本质上是保持泊松括号不变的变换，也就是把 $[\,,\,]$ 这个结构当成相空间上本质的东西。这样它就不是锦上添花，而是辛结构的化身——相空间上的函数在这之下构成一个李代数。
3. 最后是和量子力学的联系，这大概是它真正的身世。量子力学里的对易子 $[\hat{f},\hat{g}]$ 和经典泊松括号满足**完全相同的代数性质**（线性、反对称、雅可比恒等式、莱布尼茨律），于是有对应原理：
$$
[\hat{f},\hat{g}] = i\hbar \widehat{\{f,g\}}
$$
经典力学是 $\hbar \to 0$ 的极限，所以经典这边"统治"时间演化的也应当是同一个结构——泊松括号。这反过来解释了为什么力学里到处是它，也是量子化的出发点之一。
