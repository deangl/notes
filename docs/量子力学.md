...
* 再这样看问题：一个态 $\phi$ 可以正交的分成 $\psi = a_i \psi_i$。也可以按一个物理量的本征态展开为 $\psi = b_j \phi_j$。$\psi_i,\phi_j$可视为同一线性空间的单位正交基。注意$ \sum \limits_{i} \vert a_i \vert^2 =   \sum \limits_{i} \vert b_j \vert^2 = 1$ 可见$a,b$的确是**单位向量**。
$$
\begin{eqnarray*}
\bar{f} = \sum \limits_{i} b_j^{\ast} b_j f_j = B^{\ast T} F B
\end{eqnarray*}
$$
其中$F$是一个对角阵，即把本征值填到对角线上。继续考虑 $\psi$到$\phi$只差一个“正交变换” $\psi_i = \sum \limits_{j}k_{ij}\phi_j, \psi = K \phi$。知$B = KA$。从而：
$$
\begin{eqnarray*}
\bar{f} &=& B^{\ast T} F B \\
	&=& (KA)^{\ast T} F (KA) \\
	&=& A^{\ast T} (K^{\ast T} F K) A
\end{eqnarray*}
$$
