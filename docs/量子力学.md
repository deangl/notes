...
* **物理量：** 物理量测量，可以得到某些结果，而得到某结果的**概率**是粒子的波函数的一个 **“双线性”** (双线性而不是线性，是最最关键的一个点)函数，一般形式是：$\int \int \Psi(q) \Psi^{\ast}(q^{\prime})\phi(q,q^{\prime})\mathrm{d}q \mathrm{d}q^{\prime}$。在这个框架下，位置的测量也对应一个双线性函数$\phi(q, q^{\prime}) = \delta (q-q_0) \delta (q^{\prime} -q_0)$。
...
\end{eqnarray*}
$$
能量本征态下任一物理量的**平均值**不随时间变化。另外，有对任意的不显含时的$\hat{f}$：
$$
\begin{eqnarray*}
...
$$
\begin{eqnarray*}
\bar{f} &=& \sum \limits_{n} \omega_n B^{n\ast T} F B^n \\
&=&  \sum \limits_{n} \sum \limits_{j}\omega_n B^n_j F_{jj} B^n_j  = \mathrm{tr} (\Lambda(w_n) B^n B^{n\ast T} F)\\ 
&=&  \sum \limits_{n} \sum \limits_{i} \sum \limits_{j}\omega_n A^n_j F_{A ij} A^n_i = \mathrm{tr} (\Lambda(w_n) A^n A^{n\ast T} F_{A})\\
\end{eqnarray*}
$$
这里把$\omega_n B^n B^{n\ast T}$叫做密度矩阵。形式上可以表示为$\rho = \sum \limits_{n}\sum \limits_{i} \omega_n\vert i \rangle \langle i \vert $。然而上面的**依概率混合**有点混乱，什么是“概率”？难道这只是一个系综的理论？朗道的书上有另一个表述:
...
总结一下：
1. 一个纯态在一个表象下可以用 $\sum \limits_{n }a_n \vert n \rangle$来表示，为了算$f$的值，要用到 
$$
\begin{eqnarray*}
\bar{f} = \sum \limits_{j} f_j  b_j^{\ast}b_j = \sum \limits_{j} f_j (\sum \limits_{mn} a_n a_m\langle j \vert m \rangle  \langle n \vert j \rangle)\ =: \sum \limits_{mn} a_n a_m f_{mn}
\end{eqnarray*}
$$

2. 而如果是“混合态”，系统依概率$\omega_k$处于不同的纯态，那么有
$$
\begin{eqnarray*}
f = \sum \limits_{k}\omega_k (\sum \limits_{mn} a_{nk} a_{mk} f_{mn}) = \sum \limits_{mn}(f_{mn} \sum \limits_{k}\omega_ka_{nk}a_{mk} ) =: \sum \limits_{mn} \rho_{mn}f_{mn}
\end{eqnarray*}
$$
从而完成了定义。

3. 问题是，什么叫“依概率”？这个“随机”性是哪来的？有的地方，把测量当成是对于一个系综进行的，此时，“概率” 其实说的是“比例”。而在对于单体的情况下，仍是有解释的：我们在处理的不是不是状态的完备表述。

假设系统的完备表述下的态是 $\vert n,i \rangle$，其中我们所有的态只和参数$i$有关。这时就会看到，只有在同一个$n$取值的情况下，$i$才形成一个线性空间。我们只看$\vert i \rangle $的时候，就是依概率(取$n$的系统)的取到不同的态了的。
...
(\langle \phi \vert \psi \rangle)^2 = \langle \phi \vert \psi \rangle \langle 0 \vert 0 \rangle = \langle \phi \vert \psi \rangle 
\end{eqnarray*}
$$
...
### 量子纠缠
以两体态为例，两个子系统$A,B$，为之分别取一组完备的态集：$\mathcal{I} _A={i _A},\mathcal{I} _B = {i _B}$。则合系统的完备态集为 $\mathcal{I}= {(i _A,i _B)} $。且有 $ \langle j _A, j _B \vert i _A, i _B \rangle = \delta _{i _A, j _A} \delta _{i _B, j _B}$。计：

$$
\begin{eqnarray*}
\vert \psi \rangle &=& \sum \limits_{(i_A,i_B)\in \mathcal{I}} \vert i_A, i_B \rangle \langle i_A, i_B \vert \psi \rangle \\
 &=& \sum \limits_{(i_A,i_B)\in \mathcal{I}} \psi_{i_A i_B} \vert i_A \rangle \vert i_B \rangle \tag {entgl_1}
\end{eqnarray*}
$$

如果有$\psi _{i _A i _B} = \phi _{i _A} \varphi _{i _B}$，可以记 $ \vert \psi \rangle = \vert \phi \rangle \vert \varphi \rangle $。称其为“可分量子态”否则不能这么计，称其为 **“纠缠态”**。注意到对于任意只在$A$或$B$上的作用 $U _A, U _B$，都有$U _AU _B \vert \phi \rangle \vert \varphi \rangle = U _A \vert \phi \rangle U _B\vert \varphi \rangle = \vert \phi^{\ast} \rangle \vert \varphi^{\ast} \rangle$，显然还是可分的。

显然如果$A,V$的维数分别是$m,n$，那么合系统的维数应该是$m \times n$，也就是说有 $m \times n$ 个基。一个态可以用这$m \times n$ 个态来展开。再看 **entgl_1** 这个式子，可以有一个划分：

$$
\begin{eqnarray*}
\psi &=& \sum \limits_{i_A} [\vert i_A \rangle (\sum \limits_{i_B} \psi_{i_A i_B} \vert i_B \rangle)]
\end{eqnarray*}
$$

一定意义上体现了当$A$处于 $i_A$态时，我们知道 $B$ 在一个什么样的态。这个形式还可以更进一步，前面的 $m \times n$ 个基上的展开系数可以排成$m\times n$ 的矩阵$M$。由[奇异值分解](./线性代数基础.md#奇异值)，有$M = U\Sigma V^{\ast}$。这样，对$A,B$各提供了一组新的基，在这组基下：

$$
\begin{eqnarray*}
\psi &=& \sum \limits_{i}\sigma_i \vert i \rangle \vert i \rangle
\end{eqnarray*}
$$

其中 $ i \in [1, \min(m, n)] $。其中对于第一个和第二个位置中的$\vert i \rangle$都成正交基。其中不为$0$的$\sigma_i$的数目叫施密特数，可知它是确定的，最小可以取到$1$。同时可以发现取$1$时，状态是可分的。
