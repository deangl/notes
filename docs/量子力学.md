...
* 任意$f$的演化$\dot{f}_{mn} = i\omega_{mn}f_{mn} = \frac{i}{\hbar}(E_m - E_n)f_{mn}$。
...
\end{eqnarray*} 
$$
势能可无穷大，所以只有有限运动，所以只有离散的能谱。化成能量表象下的矩阵方程有：
$$
\begin{eqnarray*}
...
下面定义 $\hat{L}_+ = \hat{L}_x + i \hat{L}_y, \hat{L}_+ = \hat{L}_x - i \hat{L}_y$，有：
$$
\begin{eqnarray*}
...
考虑$\hat{L}^2 - \hat{L}_z^2 = \hat{L}_x^2 + \hat{L}_y^2$ 得到其本征值非负。可以定义$L$为给定$L^2$后 $M$ 的最大值。注意到 $\hat{L}_z \hat{L}_{\pm}\psi_M = (M\pm 1) \hat{L}_{\pm}\psi_M$，于对于$M=L$ 有$\hat{L}_z \hat{L}_+ \psi_L = (L+1) \hat{L}_+ \psi_L$，然而我们知道不存在本征值$(L+1)$的态，故有$\hat{L}_+ \psi_L=0$。

又注意到$\hat{L}_-\hat{L}_+ = (\hat{L}^2 -\hat{L}_z^2 - \hat{L}_z^2)$，作用了$\psi_L$上还是$0$，知：$L^2 = L(L+1)$。$L$称角量子数，$M = L,L-1,L-2,\cdots,-L$这$2L+1$ 个值。
