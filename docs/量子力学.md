...
* **算符：** 我们希望有一个函数 $\hat{f}$，使得有平均值 $\bar{f}$ 有关系：$ \bar{f} = \int \Psi^{\ast} (\hat{f} \Psi) dq $， 其中 $\bar{f} = \sum \limits_n f_n \lvert a_n \rvert^2 $。要注意到希望 $\hat{f}$ 并不同 $a_n$ 有关。
  ...
  * 由于物理量是实数，对$\hat{f}$ 形成了要求：
	a. $\int \Psi^{\ast}(\hat{f} \Psi) dq = \int \Psi(\hat{f}^{\ast} \Psi^{\ast})dq$ —— 实数的要求。
	b. 存在转置算符 $\tilde{\hat{f}}$，使得 $\int \Phi(\hat{f}\Psi) dq = \int \Psi (\tilde{\hat{f}}\Phi) dq$，称$\tilde{\hat{f}}$为 $\hat{f}$ 的转置。应用$\hat{f}$的推导过程之到这个转置是存在的。同时，矩阵化前面的推导过程，容易知道“转置”这个名称的由来。
