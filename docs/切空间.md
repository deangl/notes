...
一个物理量，应该是和坐标的选择无关的，我们需要这样的一个量：$p$，在已定的$L$之下，它实际上可以是切空间上的一个函数，这个函数和$L$在局域的性质有关。看如下的一个函数：$f(v) = \lim \limits_{\tau \to 0} \frac{L(u+ \tau v) - L(u)}{\tau}$，可以看到，对于每个固定的$u$，它是$v$的一个线性函数，从而对应一个余切向量。
