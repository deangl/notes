...
* 从哈密顿量来看，它被定义为 $H(q, p)$，其中 $p = \frac{\partial L}{\partial \dot{q}}$。单位由 $m$ 换成 $cm$ 时 $q_{cm} = 100 q_m$、$\dot{q}_{cm} = 100 \dot{q}_m$，由链式法则 $p_{cm} = \frac{\partial L}{\partial \dot{q}_{cm}} = \frac{\partial L}{\partial \dot{q}_m}\frac{\partial \dot{q}_m}{\partial \dot{q}_{cm}} = \frac{1}{100}p_m$，数值缩小 100 倍，是协变的，满足余切向量的要求。所以说哈密顿量是定义在余切从上的。可是这是怎么来的呢？
