...
2. 容易看出对偶空间是相互的
3. 在选定 $V$ 的一组基 $e_i$ 后，总能对应的在 $ V^* $ 找到一组 $ e^{\ast j} $ ，使得 $ e^{\ast j}(e_i) = \delta^j_i $ 。$ \forall x, f $ 在这之下的坐标为 $ x^i,f^{\ast}_{j} $ 有 $ f(x) = x^i f^{\ast}_{i}$ 。

4. 容易验证切空间和余切空间是对偶的，且可以自然的取 $e_i = \partial_i, e^{\ast j} = \pi^j$ 。其中 $\pi^j$ 为投影: $\pi^j(x) = x^j$ 。
