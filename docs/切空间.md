...
use_math: true
categories: [物理]
tags: [math,理论物理,拉格朗日,哈密顿,微分几何,分析力学,勒让德]
---
...
有一个式子，很熟了：$ \mathrm{d}f = \frac{\partial f}{\partial x^i} \mathrm{d} x^i$，对应也有沿 $\gamma(t)$ 的方向导数 $\frac{\mathrm{d}}{\mathrm{d}t} f(\gamma(t)) = \partial_i f \xi^i$。重新理解一下:
...
* 证明两函数切向量之比等于其导数之比。设 $f, g$ 在 $p$ 处满足 $\partial_i g(p) = \lambda \partial_i f(p)$（$\lambda$ 即"导数之比"），则 $g - \lambda f$ 在 $p$ 处偏导数为零，由上一条得 $v(g-\lambda f) = v(g) - \lambda v(f) = 0$，于是 $\frac{v(g)}{v(f)} = \lambda$。
* 还有一步比较繁琐的是，证明它是方向导数，这需要把一个函数$f$在$p$的邻域的一个坐标卡$(x^i), p \rightarrow (0)$上展开，再用上面的性质可以推到：$v (f) = v (x^i)\frac{\partial f}{\partial x^i}(0) $。是一个方向导数。
...
所以，直观的，**切向量是自变量空间的方向箭头，余切向量是自变量$\times$因变量空间的斜面**，如下图所示。易知，在取两个基$\partial_x , \partial_x^{\prime}$下，切向量 $v$ 的数值变化是和基的 “长度反比的”，而余切向量 $ v^*$ 是用基对应的函数值的变化来表示的，所以其值和基的“长度正比”的 <a name="切和余切图像"></a>。
![](./img/1692005047.png)
...
* 设$ \mathrm{d} f =\sum \limits_{j} a_j \mathrm{d} x^j $，有$ (df)(\frac{\partial }{\partial x^i}) = \sum \limits_{j} a_j (\mathrm{d} x^j) (\frac{\partial }{\partial x^i}) = \sum \limits_{j} a_j \delta_i^j = a_i$
...
理清几个点。
1. 不要想完全的脱离 “坐标” 来理解，否则不用在流形上来讨论。
2. 切向量和余切向量，对应的其实是两类元素的局部线性化的等价类：切向量是过一个点的曲线方向的等价类，余切向量是函数的函数芽的等价类。
...
* 从哈密顿量来看，它被定义为 $H(q, p)$，其中 $p = \frac{\partial L}{\partial \dot{q}}$。单位由 $m$ 换成 $cm$ 时 $q _{cm} = 100 q _m$、$\dot{q} _{cm} = 100 \dot{q} _m$，由链式法则 $p _{cm} = \frac{\partial L}{\partial \dot{q} _{cm}} = \frac{\partial L}{\partial \dot{q} _m}\frac{\partial \dot{q} _m}{\partial \dot{q} _{cm}} = \frac{1}{100}p _m$，数值缩小 100 倍，是协变的，满足余切向量的要求。所以说哈密顿量是定义在余切从上的。可是这是怎么来的呢？
...
假设在 $M$ 上有一点$x$，在一个坐标卡下（把邻域看成 $\mathbb{R}^n$）定义 $D_v f(x) := \frac{\mathrm{d} }{\mathrm{d} t} f(x+tv) = v^i \frac{\partial f}{\partial x^i}(x)$。而我们前面就说过切向量是方向导数：$v(f) := D_v(f)$，这样，我们知道了$v = v^i \partial_i$，是切向量。其中$v^i$ 是坐标, $\partial_i$是基。
...
它守恒好理解：由哈密顿方程 $\frac{\partial H}{\partial q} = -\frac{\mathrm{d} p}{\mathrm{d} t}$、$\frac{\partial H}{\partial p} = \frac{\mathrm{d} q}{\mathrm{d} t}$，得 $ \frac{\mathrm{d} H}{\mathrm{d} t} = \frac{\partial H}{\partial q} \frac{\mathrm{d} q}{\mathrm{d} t} + \frac{\partial H}{\partial p} \frac{\mathrm{d} p}{\mathrm{d} t} = 0$。若 $H$ 显含 $t$，还须加上 $\frac{\partial H}{\partial t}$ 项，此时 $\frac{\mathrm{d} H}{\mathrm{d} t} = \frac{\partial H}{\partial t}$，一般不再守恒。但是为什么就这么碰巧就不好理解了。
